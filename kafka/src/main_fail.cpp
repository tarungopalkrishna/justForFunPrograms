...
//             exit(1);
//         }
//         if (rd_kafka_conf_set(conf, "bootstrap.servers", "localhost:9092", err_string, sizeof(err_string)) !=
//         RD_KAFKA_CONF_OK){
//             fprintf(stderr, "%% %s\n", err_string);
//             exit(1);
//         }
...
//         if (rd_kafka_conf_set(rk_conf, "client.id", "localhost:9092", err_string, sizeof(err_string)) !=
//         RD_KAFKA_CONF_OK) {
//             fprintf(stderr, "%% -> %s\n", err_string);
//             exit(1);
//         }
//         if (rd_kafka_conf_set(rk_conf, "bootstrap.servers", "localhost:9092", err_string, sizeof(err_string)) !=
//         RD_KAFKA_CONF_OK){
//             fprintf(stderr, "%% %s\n", err_string);
//             exit(1);
//         }
...
//         // if (rd_kafka_topic_conf_set(topic_conf, "acks", "all", err_string, sizeof(err_string)) !=
//         RD_KAFKA_CONF_OK) {
//         //     fprintf(stderr, "%% %s\n", err_string);
//         //     exit(1);
//         // }
...
void test_consumer(std::string brokers, std::string topic, int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "g:b:z:qd:eX:AM:qv")) != -1) {
        switch (opt) {
            ...
            case 'z':
                ...
                if (conf->set("compression.codec", optarg, error_string) != RdKafka::Conf::CONF_OK) {
                    std::cerr << error_string << std::endl;
                    exit(1);
                }
                break;
            ...
            case 'M':
                ...
                if (conf->set("statistics.interval.ms", optarg, error_string) != RdKafka::Conf::CONF_OK) {
                    std::cerr << error_string << std::endl;
                    exit(1);
                }
                break;
    ...
    for (; optind < argc; optind++) topics.push_back(std::string(argv[optind]));
    ...
    usage:
        fprintf(stderr,
                ...
                "\n"
                "\n",
                argv[0], RdKafka::version_str().c_str(), RdKafka::version(), RdKafka::get_debug_contexts().c_str());
        exit(1);
    }
...
RdKafka::KafkaConsumer *get_consumer(bool is_valid, int argc, char **argv) {
    std::string error_string;
    std::string mode;
    std::string debug;
    std::string brokers;
...
int main(int argc, char **argv) {
    ...
    if (topics.size() > 0) {
        ...
        std::cout << "The size of the topics is:" << topics.size() << std::endl;
    }

    RdKafka::KafkaConsumer *consumer = get_consumer(true, argc, argv);
    // std::cout << "The type of consumer is: " << typeid(consumer).name() << std::endl;
